...
const MonitorGrid = () => {
  ...
  useEffect(() => {
    ...
    if (!revealCompleted && breakoutCompleted) {
      ...
      // Initial state: fixed position, circularly masked.
      // IMPORTANT: no x/y translation is used for cards.
      // The reveal origin is the bot face, so each card appears to
      // emerge from behind the bot instead of opening from its own center.
      ...
      cards.forEach((card) => {
        ...
        const originX =
          ((botCenterX - cardRect.left) / cardRect.width) * 100;
        const originY =
          ((botCenterY - cardRect.top) / cardRect.height) * 100;
        ...
        onComplete: () => {
          ...
          cards.forEach((card) => {
            gsap.set(card, {
              ...
              clipPath: `circle(220% at ${revealOrigins.get(card) || "50% 50%"})`,
              scale: 1,
              opacity: 1,
            });
          });
      ...
      // Short cinematic pause after the bot settles.
      revealTimeline.add(() => {}, 0.45);

      // ==========================================================
      // CLOCKWISE ORDER AROUND THE BOT
      // ==========================================================
      //
      //       0 UPTIME        1 ALERTS
      //             \        /
      ...
      cardOrder.forEach((cardInfo, seqIndex) => {
        ...
        // Skip cards hidden by responsive Tailwind classes.
        ...
        revealTimeline.add(() => {
          ...
          gsap.fromTo(
            ...
            {
              boxShadow:
                "0 0 16px #75ff9e, 0 0 30px rgba(117,255,158,0.9)",
              duration: 0.12,
              yoyo: true,
              repeat: 1,
              stagger: 0.025,
        ...
        // ----------------------------------------------------------
        // 3. CIRCULAR CARD REVEAL
        // ----------------------------------------------------------
        revealTimeline.to(
          card,
          {
            // Reveal outward from the bot face while the card itself
            // remains completely stationary at its CSS position.
            clipPath: `circle(220% at ${revealOrigins.get(card) || "50% 50%"})`,
            scale: 1,
            duration: 0.48,
            ease: "power2.out",
        ...
        // Slight content activation / glow.
        revealTimeline.to(
          card,
          {
        ...
        // Return to the normal card state before the orbit continues.
        revealTimeline.to(
          card,
          {
