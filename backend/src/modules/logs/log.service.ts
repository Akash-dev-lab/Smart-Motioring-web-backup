// src/modules/logs/log.service.ts
import mongoose from "mongoose";
import Log from "./log.model.js";
import type {
  MonitorAnalyticsRange,
  MonitorAnalyticsResult,
  AnalyticsAggregationResult,
} from "./types/index.js";

export const getMonitorAnalytics = async (
  monitorId: string,
  range: MonitorAnalyticsRange | string = "24h"
): Promise<MonitorAnalyticsResult> => {
  // ⏱ time window
  const now = new Date();
  let from = new Date(now.getTime() - 24 * 60 * 60 * 1000); // default 24h
  if (range === "1h") from = new Date(now.getTime() - 60 * 60 * 1000);

  const result = await Log.aggregate<AnalyticsAggregationResult>([
    {
      $match: {
