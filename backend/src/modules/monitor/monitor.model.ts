...
const monitoringTargetSchema = new Schema<IMonitoringTarget>(
  {
    region: {
...
const monitorSchema = new Schema<IMonitorDocument>(
  {
    ...
    monitoringTargets: {
      ...
      default: [],
      validate: {
        validator: (targets: IMonitoringTarget[]) => {
          const regionIds = targets.map((target) =>
            target.region?.toString()
          );
...
const Monitor: Model<IMonitorDocument> =
